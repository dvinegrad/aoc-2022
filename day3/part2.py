...
# Code Golf
print(sum(map(lambda c: ord(c) - 96 if c.islower() else ord(c) - 38, [next(iter(group[0] & group[1] & group[2])) for group in [[set(l) for l in lines[i:i+3]] for i in range(0, len(lines), 3)]])))